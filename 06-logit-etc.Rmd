...
4. Something with $x$ having a mean of zero and thinking about different changes from there. Different values of $\alpha$ etc. Think about relating this to the prior in the Bayes' rule calculation.
5. Something with odds *ratios*.
6. Something with a generic logit regression $X'\beta$; something about interactions being really messy?


## Odds and Ends about Risk
![](https://imgs.xkcd.com/comics/spacecraft_debris_odds_ratio.png)

Logistic regression is often used to talk about *risk*. For example: what is the risk that a baby born to a mother with characteristics $X$ will have a low birthweight? How does this risk vary across mothers with different characteristics? There are many different ways to pose this questions, and it's extremely important not to mix them up. Confusion on this score is extremely common and potentially serious, as it can lead people to give too much weight to tiny risks and not enough weight to serious ones.
...
$$
\text{Relative Risk} \equiv \frac{P(Y=1|X=x_2)}{P(Y=1|X=x_1)}
$$
