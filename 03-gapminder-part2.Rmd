...
In this lesson we'll build on your knowledge of `dplyr` and the `gapminder` dataset and introduce [`ggplot2`](https://ggplot2.tidyverse.org/), the R graphics package *par excellence*. Like `dplyr`, `ggplot2` is also a part of the Tidyverse family of packages. To install the whole family of packages, use `install.packages('tidyverse')`. To load all of them at once, use `library(tidyverse)`. (Of course you can also install and load `ggplot2` on its own if you prefer.) This lesson is only the tip of the iceberg when it comes to `ggplot2`. To learn more, see [R for Data Science](https://r4ds.had.co.nz/) is and the [`ggplot2` cheat sheet](https://github.com/rstudio/cheatsheets/blob/master/data-visualization-2.1.pdf).
...
Dot plots are typically most informative when *sorted* by the continuous variable in the plot, `meanLifeExp` in our case. A convenient way to achieve this is by using the `fct_reorder()` function from the [`forcats` package](https://forcats.tidyverse.org/), a member of the [Tidyverse](https://www.tidyverse.org/). Here's a simple pipeline that does the trick:
```{r}
library(forcats)
by_continent %>%
...
gapminder %>%
  ...
  summarize(meanLifeExp = mean(lifeExp)) %>%
  ggplot() +
  geom_point(aes(x = meanLifeExp, y = continent, color = year))
```
Some more examples <https://uc-r.github.io/cleveland-dot-plots>
