...
![Commander Data in a Western Landscape (the only public domain image I could find!)](https://upload.wikimedia.org/wikipedia/commons/7/7d/Glenn_Cote_Brent_Spiner.jpg)

By this stage you've read a number of different datasets into R and done a bit of data [wrangling](https://en.wikipedia.org/wiki/Wrangler_(profession)) with `dplyr`. Now it's time for a full-fledged data [rodeo](https://en.wikipedia.org/wiki/Rodeo)! This goal of this lesson is to introduce you to more complicated data wrangling issues and more powerful tools to solve them. We'll dig deeper into `dplyr` to automate tedious tasks, we'll learn how to *merge* datasets, we'll see how [`tidyr`](https://tidyr.tidyverse.org/) can be used to *pivot* datasets between long and wide formats, we'll learn a bit more about the various kinds of "pipes" that are available in R, and we'll learn how to overcome the many challenges of importing data from Microsoft Excel.
...
```{r}
# Select columns that contain the abbreviation of your favorite course
gradebook %>%
  select(contains('erm'))
```
...
### Exercise
1. An example from the vignette!
2. Complete the merge described above. What type of join makes sense here?
...
## Addendum: Put that in your pipe and smoke it!
