...
Besides the basic properties of expected value, variance, and covariance, there's only one other piece of information that we'll need to generate correlated normals: **the sum of independent normals is normal.** Now we're ready to start building everything up from first principles!
...
**The correlation matrix contains *strictly less information* than the variance-covariance matrix. Knowledge of the correlation matrix isn't enough to construct the variance covariance matrix.**
2. Read in the `kids` dataset from `https://ditraglia.com/econ103/child_test_data.csv` and calculate the covariance matrix of `mom.iq` and `kid.score` and the correlation matrix of the same.
```{r, message = FALSE}
library(tidyverse)
...
Notice that `geom_density()` works in effectively the same way as `geom_histogram()` but the output is different. The preceding two sets of plots show the **marginal distributions** of `z1` and `z2`. This tells us how each random variable behaves *taken by itself*. To see how `z1` and `z2` covary, we need to examine the *joint distribution*. A two-dimensional kernel density plot is helpful way to do this. Rather than trying to plot a 3d surface, we'll take the "view from above," draw in contour lines, and add a color scale using `geom_density2d_filled()`:
```{r}
ggplot(data.frame(z)) +
...
### Exercise
1. Reading from the color scale, the height of the "peak" in each of the two-dimensional kernel density plots from above was around 0.16. Why is this?
2. Continuing from the exercise from above, create both marginal and joint kernel density plots of `mom.iq` and `kid.score`. Do these appear to be normally distributed? 
```{r, message = FALSE}
ggplot(kids) +
...
\end{aligned}
$$
This means it's perfectly fine to work with mean zero normals in the rest of the lesson. After we've created normal random variables with the desired variances and covariances, we can always shift their means as needed after the fact. To change the *variances* of $Z_1$ and $Z_2$ without creating any covariance between them, we simply multiply each by a constant. While we won't delve into the details of the geometry, this turns *circles* of equal density into *ellipses*. For example:
```{r}
...
An equivalent, but slightly nicer way of constructing `x` uses matrix multiplication. Written in matrix from, the two linear combinations from above can be expressed as
$$
\begin{bmatrix} X_1 \\ X_2 \end{bmatrix} = \mathbf{A} \begin{bmatrix} Z_1 \\ Z_2\end{bmatrix}, \quad \mathbf{A} \equiv \begin{bmatrix} 2 & 1 \\ 1 & 4\end{bmatrix}.
$$
To implement this in R, we need a few new commands. First, `matrix()` can be used to convert a vector into a matrix:
```{r}
A <- matrix(c(2, 1, 
...
Next we need to multiply $(Z_1, Z_2)'$ by $\mathbf{A}$. Each *row* of the matrix `z` contains one simulated realization of $(Z_1, Z_2)'$ but our expression above arranges these random variables in a column vector. This means we'll need to transpose them using the `t()` function. Here's an example of `t()` in action:
```{r}
A
t(A)
```
Finally, we need to know how to multiply matrices in R. There are *two* ways of doing this. The first is *elementwise* multiplication: `A * B` multiplies every element of `A` by the corresponding element of `B` for example
```{r}
A * A
```
What we need here is the second form of multiplication: *matrix* multiplication: `A %*% B` carries out the usual matrix multplication operation that we know and love from our linear algebra training. For example:
```{r}
A %*% A
```
...
Notice that the correlation between $X_1$ and $X_2$ produces a *tilted ellipse* in the joint density plot. The ellipse slopes upward because $X_1$ and $X_2$ are positively correlated. So how could we have worked out in advance what correlations we'd end up with in this example? Indeed we could! You'll work out how in the next exercise.
...
<div style= "float:left;position: relative; margin-right: 10px; height: 30%;">
![André-Louis Cholesky (1875-1918)](https://upload.wikimedia.org/wikipedia/commons/5/5f/Andre_Cholesky.jpg)
</div>

### Going Backwards
But we *still* don't know how to generate correlated normal draws! For example, suppose we require want generate two standard normals $X_1$ and $X_2$ with $\text{Cor}(X_1, X_2) = 0.5$. How can we achieve this? In effect, this requires us to work through the preceding exercise *in reverse*. That is, we need to find constants $a, b, c$, and $d$ such that  
$$
\begin{aligned}
X_1 &= a Z_1 + b Z_2 \\
X_2 &= c Z_1 + d Z_2
\end{aligned}
$$
gives $\text{Var}(X_1) = \text{Var}(X_2) = 1$ and $\text{Cor}(X_1, X_2) = 0.5$. There are in fact *multiple* choices of $a, b, c$, and $d$ that will do the trick. But there's one choice that is particularly simple and convenient. 

Suppose we take $a = 1$ and $b = 0$, so that $X_1 = Z_1$. This ensures that $\text{Var}(X_1) = 1$. Now the question becomes: how can we choose $c$ and $d$ so that the variance of $X_2$ is *also* one and the correlation between $X_1$ and $X_2$ equals 0.5. Using the calculations from the exercise above, $\text{Var}(X_2) = c^2 + d^2$ so we need to set $c^2 + d^2 = 1$. Solving, this requires $d \pm \sqrt{1 - c^2}$. Again let's keep things simple and choose the positive solution for $d$. Again using the calculations from the exercise, $\text{Cov}(X_1, X_2) = ac + bd$ but since we've set $a = 1$ and $b = 0$ this simplifies to $\text{Cov}(X_1, X_2) = c$. Therefore $c = 0.5$. Expressed in matrix form, we've shown that
$$
...
$$
And indeed this gives us the desired result: 
```{r}
A <- matrix(c(1, 0,
...
$$
3. Check your calculations in the preceding part by transforming the simulations `z` from above into `x` such that `x1` has variance one, `x2` has variance four, and the correlation between them equals 0.4.
```{r}
...
There are two important facts about square roots. First, they don't always exist: there's no real number that equals $\sqrt{-4}$. Second, they're not necessarily unique: both $-2$ and $2$ when squared equal four. So is it always possible to write a variance-covariance matrix in the form $\mathbf{A}\mathbf{A}'$? The answer turns out to be *yes*. 

Variance-covariance matrices are symmetric, but not every symmetric matrix is a variance-covariance matrix. This matrix, for example is definitely *not* a valid variance-covariance matrix:
$$
\begin{bmatrix}
4 & 16 \\
16 & 9
\end{bmatrix}
$$
Why not? Converting it to a correlation matrix gives nonsense:
```{r}
M <- matrix(c(4, 16, 
              16, 9), byrow = TRUE, nrow = 2, ncol = 2)
cov2cor(M)
```
Correlations definitely shouldn't be larger than one! In the same way that variances must be positive, variance-covariance matrices must be *positive definite*. A symmetric matrix $\mathbf{M}$ is called *positive definite* if $\mathbf{v}' \mathbf{M} \mathbf{v}>0$ for any vector $\mathbf{v} \neq \mathbf{0}$. This definition is effectively the matrix equivalent of a positive number. To see why it makes sense, suppose that $\mathbf{X} \sim \text{N}(\mathbf{0}, \boldsymbol{\Sigma})$ where
$$
...
2. Tell them how to check positive definiteness. Have them check two matrices: one is and one isn't! Tell them to use `det` 
3. Generate three correlated normals using `chol()`. Check your work with `cov()`. **Something with three correlated normals. Also something to check positive definiteness?**
4. Now do it *the easy way* using `rmvnorm()`. Again check your work with `cov()`.
