...
# not run
tidy_smith %>%
  with(wordcloud(word, n, min.freq = 400))
```
...
As we saw above, `unnest_tokens()` does a fair amount of clean-up by default, removing punctuation and capitalization. But sometimes we may want to carry out a bit of additional clean-up before carrying out our text analysis. Let's look at the top thirty tokens from `tidy_smith`:
```{r}
...
$$
where $\alpha>0$ is the parameters of the distribution, $C$ is the normalizing constant that makes the probabilities sum to one over $x = 1, 2, ...$ namely
$$
...
1. Read `count_1w.txt` into a tibble called `norvig` using `read_delim()` from the `readr` package. I suggest setting meaningful column names using the argument `col_names`.
2. Use `norvig` to make a plot with the log rank on x-axis and log count on the y-axis. If these data come from a Zipf distribution, what shape should this relationship take? 
3. Fit an appropriate linear regression to assess whether the counts and ranks from `norvig` are consistent with Zipf's law.
...
The **term frequency** (tf) of a token is a measure of how often this token appears in a given document. Let's count term frequency of every word in the complete novels of Jane Austen. To start we'll load these novels using the [`janeaustenr` package](https://github.com/juliasilge/janeaustenr):
```{r}
library(janeaustenr)
austen_books()
```
...
$$
Imagine a corpus of academic papers written in English, spanning all disciplines from Archaeology to Zoology. The term "the" will certainly appear in every one of these papers, so its idf is zero. Now imagine a corpus of academic papers. In contrast, the term "heteroskedasticity" will only appear in a small fraction of the papers, those that concern econometrics or statistics in some way. The idf of this term is high. Encountering the term "the" tells us effectively nothing about the kind of document that we're reading; encountering the term "heteroskedasticity" tells us a lot. While this is only a heuristic, it's a useful one. And it becomes even more useful when *combined* with the notion of term frequency.

A term that is both *common in a given document*, high tf, and is *only contained in a few documents* is a good candidate for an important word. This is the idea that tf-idf tries to capture by *multiplying* term frequency by inverse document frequency. We can use the `tidytext` function `bind_tf_idf()` to attach term frequency, inverse document frequency, and tf-idf to `austen_words` as follows:
```{r}
austen_words %>%
  bind_tf_idf(word, book, n) 
```
The first argument to `bind_tf_idf()` is a tidy text dataset (a tibble) with *one row per term per document*. In our case, this is `austen_words`, supplied using the pipe. The second argument is `term`, the name of the column of the input tibble that contains our terms. Above, this is the `word` column of `austen_words`. The next argument is `document`, the name of the column in our input tibble that cotains the document ids. In our case this is `book`. Finally, `n` is the column of our input tibble that contains the *counts* of each term by document. 
...
[Latent Dirichelt Allocation](https://www.jmlr.org/papers/volume3/blei03a/blei03a.pdf?ref=https://githubhelp.com), or LDA for short, is an extremely popular method for "topic modelling," 
...
A DTM is a matrix that summarizes the frequency of particular words (terms) within a corpus of documents. Each row is a document and each column is a word. The $(i,j)$ entry in the matrix tells us how many times word $j$ occurs in document $i$. Remember: it's arranged as $(\text{document}, \text{word})$. See also: <https://en.wikipedia.org/wiki/Document-term_matrix>. DTMs are an extremely common format in packages for analyzing text data. They are also *extremely inconvenient* for doing anything other than feeding data into such packages! Store in a tidy format, and then convert to DTM if needed <https://www.tidytextmining.com/dtm.html?q=document%20term#tidying-documenttermmatrix-objects>



### Pre-processing
Want to create a *term-document matrix*. (What is this?) But first we need to do some pre-processing:


### Using a Dictionary

What does all this pre-processing do for us? Dramatically reduces the dimension of the problem because we end up with many fewer *terms* than we started with: stop words are removed, similar words are stemmed to be the same word, capital letters converted to lowercase etc. But still, we typically have way too many words after pre-processing! What to do about this? Use a **dictionary**. List the words we're interested in, and restrict attention to thses. An example is a **sentiment dictionary** <https://www.tidytextmining.com/sentiment.html?q=dictionary#comparing-the-three-sentiment-dictionaries>


### Cosine Similarity
There's an example in Imai that we could use here.
