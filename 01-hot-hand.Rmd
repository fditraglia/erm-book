...
In economics and statistics, simulation is a *superpower*. It helps us to understand our models, check for mistakes, and make unexpected connections, some of which may even lead to new theoretical results. Riffing on the two quotes from above, simulation is both a substitute for and a complement to cleverness. If you'll pardon my continuation of the Swiss Army Knife metaphor, simulation is the knife: arguably the most useful tool in your toolbox. In this lesson, we'll learn the basics of carrying out a simulation study in R using the following key commands: 
...
1. Write a function called `get_avg_after_streak()` that takes a single input argument: `shots` a vector of zeros and ones. Your function should work as follows. First, identify all "streaks" in `shots`. Streaks are defined as a sequence of three ones in a row. Next, extract the elements of `shots` that immediately *follow* a streak, storing them in a vector called `after_streak`. Finally, compute the mean of `after_streak`. Test your function on the vector `c(0, 1, 1, 1, 1, 0, 0, 0)` to make sure that it's working correctly.
```{r, webex.hide = 'Show Solution'}
get_avg_after_streak <- function(shots) {
  stopifnot(all(shots %in% c(0, 1)))
  after_streak <- rep(NA, length(shots) - 3)
  for(i in 1:length(after_streak)) {
    after_streak[i] <- all(shots[i:(i + 2)] == 1)
  }
  after_streak <- c(rep(FALSE, 3), after_streak)
  after_streak <- shots[after_streak]
  mean(after_streak)
}
...
`r hide("Show Solution")`
Amos is wrong: the expected value of his test statistic is approximately `0.46` rather than `0.5`, so he will on average conclude that Liz exhibits a *cold hand*. This approach is biased *against* finding evidence of a hot hand if it exists: 
```{r}
set.seed(420508570)
sims <- replicate(1e4, get_hot_handedness(100, 0.5))
mean(sims, na.rm = TRUE) # It's possible, though unlikely, 
# that get_hot_handedness() will return an NaN. This happens
# when the sequence of 100 simulated basketball shots has no
# streaks of length three.
```
`r unhide()`
5. **Bonus Question:** How do the results of the preceding exercise vary with Liz's success rate and the number of shots that Amos watches her take? Design a simulation study to answer this question by exploring a grid of values for these parameters. 

6. **Bonus Question:** Repeat the preceding exercise for different definitions of a *streak*, e.g. two successes in a row or four successes in a row. Think about which functions you'll need to change and how.
